...
export const Pause = ({ className = '' } = { className: '' }) => (
...
export const Play = ({ className = '' } = { className: '' }) => (
...
const VolumeControl = () => {
  ...
  return (
    <div className="flex justify-center gap-x-2">
      ...
      <Slider value={[volume * 100]} max={100} min={0} className="w-[95px]" onValueChange={(value) => {
...
const SongControl = ({ audio }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      audio.current?.removeEventListner('timeupdate', handleTimeUpdate);
    }
  }, []);
  ...
  return (
    <div className="flex gap-x-3 text-xs pt-2 text-right">
      <span className="opacity-50 w-12">{formatTime(audio.current?.currentTime)}</span>
      <Slider defaultValue={[0]} value={[currentTime]} min={0} max={audio?.current?.duration ?? 0} className="w-[200px]" onValueChange={(value) => {
        audio.current.currentTime = value;
      }} />
      <span className="opacity-50 w-12">{formatTime(audio.current?.duration ?? 0)}</span>
    </div>
  )
}
...
export function Player() {
  const { isPlaying, setIsPlaying, volume, currentMusic } = usePlayerStore(state => state)
